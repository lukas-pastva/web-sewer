		...
		if($_FILES['foto']['size'] > 307200){
			...
			if($_FILES['foto']['tmp_name']){
				if(mb_substr($_FILES['foto']['name'],-4)=='.jpg'){
					...
					psw_mysql_query($sql='
						INSERT INTO odkaz (alt, image, link, position) VALUES 
						("'.$_REQUEST['_alt'].'", "'.$uploadedImage.'", "'.$_REQUEST['_link'].'", "'.$_REQUEST['_position'].'") ');
				...
				psw_mysql_query($sql='
						INSERT INTO odkaz (alt, image, link, position) VALUES 
						("'.$_REQUEST['_alt'].'", "no image", "'.$_REQUEST['_link'].'", "'.$_REQUEST['_position'].'") ');
			}
		}
	}
	/*********************************************************************************************/
...
if($_REQUEST['state']!='detail'){
	echo '
<br />
<form action="' .$_SERVER['PHP_SELF']. '?sekcia=odkazy" method="post" enctype="multipart/form-data">
<table>
 <tr>
  <td>
 ...
 <tr>
  ...
  <td>
   <input type="text" name="_position" value="' .validateForm($_REQUEST['_position']). '" size="20" />
  </td>
 </tr>
 <tr>
  <td colspan="2">
...
';
	...
	//getall rows
	$odkazy = getTableRows('odkaz','','position ASC');
	...
	foreach($odkazy as $key => $odkaz){
		...
		echo '<td>'.($odkaz['odkaz_id']).': <a href="'.$_SERVER['PHP_SELF'].'?sekcia=odkazy&state=detail&odkaz_id='.$odkaz['odkaz_id'].'" title="Upraviť"><img src="../image/2/'.$odkaz['odkaz_id'].'.jpg" style="border: 1px solid #666666;" /></a></td>
		      <td>'.$odkaz['alt'].'</td>
		      <td>'.$odkaz['link'].'</td>
		      <td>'.$odkaz['position'].'</td>';
		echo '</tr>';
...
<table>
 ...
 <tr>
  ...
  <td>
   <input type="text" name="_position" value="' .validateForm($data['position']). '" size="20" />
  </td>
 </tr>
 <tr>
  <td colspan="2">
