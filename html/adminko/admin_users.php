...
<table border="1" bordercolor="black"
	style="border-width: 2px; border-color: black; border-style: solid; background-color: #bbbbbb;">
	<tr>
	...
	<form action="index.php?sekcia=users&amp;x=1" method="post">
	
	
	<tr>
		<td align="center"><input type="hidden" name="nick"
			value="' .$_SESSION['meno_uzivatela']. '"> '.$_SESSION['meno_uzivatela'].'
		</td>
...
<table border="1" bordercolor="black"
	...
	for( $i=0; $i<mysql_num_fields($fields); $i++ ){
		if( (mysql_field_name($fields, $i) != "id") && (mysql_field_name($fields, $i) != "pass") ){
			echo '
		<td align="center">&nbsp;<b>' .(
			substr(mysql_field_name($fields, $i),0,4)=='str_'?
			'str_'.getStructureNameFromId(substr(mysql_field_name($fields, $i),4)):mysql_field_name($fields, $i)
			). '</b>&nbsp;
		</td>';
	...
	while($one_user = psw_mysql_fetch_array($all_users)){
		echo '
	<tr>
		<form action="index.php?sekcia=users&amp;x=5" method="post"><input
			type="hidden" name="id" value="' .$one_user['id']. '"> ';
		for( $i=0; $i<mysql_num_fields($fields); $i++ ){
			if( (mysql_field_name($fields, $i) != "id") && (mysql_field_name($fields, $i) != "pass") ){
				if(mysql_field_name($fields, $i) != "nick"){
					echo '<td align="center"><input type="checkbox" 
						   name="' .mysql_field_name($fields, $i). '"
						 '.($one_user[mysql_field_name($fields, $i)] == "0"?'':'checked="checked"').' ></td>';
				} else {
					echo '
		<td align="center">&nbsp;<i>' .$one_user[mysql_field_name($fields, $i)]. '</i>&nbsp;
		</td>';
				}
			}
		}
		...
		if ($_SESSION['meno_uzivatela'] != $one_user['nick']){
			echo '
		<form action="index.php?sekcia=users&amp;x=3" method="post">		
		<td><input type="hidden" name="id" value="' .$one_user['id']. '">
...
<form action="index.php?sekcia=users&amp;x=2" method="post">
<table border="1" bordercolor="black"
	style="border-width: 2px; border-color: black; border-style: solid; background-color: #bbbbbb;">
	<tr>
		<td align="center"><b>meno</b></td>
	...
	for( $i=0; $i<mysql_num_fields($fields); $i++ ){
		if( (mysql_field_name($fields, $i) != "id") && (mysql_field_name($fields, $i) != "pass") && (mysql_field_name($fields, $i) != "nick") ){
			echo '
		<td align="center">&nbsp;<b>' .(substr(mysql_field_name($fields, $i),0,4)=='str_'?'str_'.getStructureNameFromId(substr(mysql_field_name($fields, $i),4)):mysql_field_name($fields, $i)). '</b>&nbsp;
		</td>';
	...
	<tr>
		...
		<td align="center"><input type="password" name="pass_1" size="10"></td>
		<td align="right"><input type="password" name="pass_2" size="10"></td>';

	for( $i=0; $i<mysql_num_fields($fields); $i++ ){
		if( (mysql_field_name($fields, $i) != "id") && (mysql_field_name($fields, $i) != "pass") && (mysql_field_name($fields, $i) != "nick") ){
			echo '
		<td align="center"><input type="checkbox"
			name="' .mysql_field_name($fields, $i). '"></td>';
